...
const Plans: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'wishlist' | 'planner'>('wishlist'); // Состояние для переключения между wishlist и planner
    ...
    <div className={classes.container}>
      ...
      <div className={classes.tabNavigation}>
        <div className={classes.tabButtons}>
          <button
            className={`${classes.tabButton} ${activeTab === 'wishlist' ? classes.activeTab : ''}`}
            onClick={() => setActiveTab('wishlist')}
          >
            {translations.myWishlist[language]}
          </button>
          <button
            className={`${classes.tabButton} ${activeTab === 'planner' ? classes.activeTab : ''}`}
            onClick={() => setActiveTab('planner')}
          >
            {translations.planATrip[language]}
          </button>
        </div>
      </div>
        ...
        <>
          <div className={classes.header}>
            <div className={classes.headerLeft}>
              <h1>{translations.myWishlist[language]}</h1>
